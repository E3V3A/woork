...
*/
	...
	// 
        String query = String.format("SELECT * FROM %s WHERE %s=%d AND %s=%d AND %s=%d AND %s=%d",
                ...
                DBTableColumnIds.DBI_MEASURE_GPSD_LAT,                lat,	//"gpsd_lat"
                DBTableColumnIds.DBI_MEASURE_GPSD_LON,                lng,	//"gpsd_lon"
                DBTableColumnIds.DBI_MEASURE_RX_SIGNAL,                signal);	//"rx_signal"
 ...
 */
    ...
    public void insertDefaultLocation(String country,
        ...
        //TODO I'm not writing to DB yet do I need to Check if exists? and what will we check country and mcc?
    }
    ...
    public boolean insertDBeImport(String db_src,
        ...
        String query = String.format("SELECT * FROM %s WHERE %s = %d AND %s = %d ",
                DBTableColumnIds.DBE_IMPORT_TABLE_NAME,
                DBTableColumnIds.DBE_IMPORT_LAC,
                lac,
                DBTableColumnIds.DBE_IMPORT_CID,
                cid);
    ...
    public void insertBTS(Device device){
        ...
        if(!cellInDbiMeasure(device.mCell.getCID())){
            ...
            dbiMeasure.put(DBTableColumnIds.DBI_MEASURE_GPSD_LAT, String.valueOf(device.mCell.getLat()));
            dbiMeasure.put(DBTableColumnIds.DBI_MEASURE_GPSD_LON, String.valueOf(device.mCell.getLon()));
            ...
            dbiMeasure.put(DBTableColumnIds.DBI_MEASURE_BER,(int)device.mCell.getBearing());
    ...
    public boolean insertDetectionStrings(String detection_string,
        ...
        String query = String.format("SELECT * FROM %s WHERE %s = \"%s\" AND %s = \"%s\"",
                ...
                DBTableColumnIds.DETECTION_STRINGS_LOGCAT_STRING,
                detection_string,
                DBTableColumnIds.DETECTION_STRINGS_SMS_TYPE,
                sms_type);
    ...
    public boolean insertSmsData(String time,
        ...
        String query = String.format("SELECT * FROM %s WHERE %s = \"%s\"",
                DBTableColumnIds.SMS_DATA_TABLE_NAME,
                DBTableColumnIds.SMS_DATA_TIMESTAMP,
                time);
