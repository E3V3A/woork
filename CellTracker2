...
//            Log.i(TAG, mTAG + ": ALERT: No neighboring cells detected for CID: " + mDevice.mCell.getCID() );
    ...
    public void onLocationChanged(Location loc) {
        ...
        if (loc != null && (loc.getLatitude() != 0.0 && loc.getLongitude() != 0.0)) {
            ...
            if (mTrackingCell) {
                ...
                //WILL REMOVE JUST LEAVING FOR REF NOW
                     /*   Device.getNetworkTypeName(mDevice.mCell.getNetType()),
                        mDevice.mCell.getMCC(),
                        mDevice.mCell.getMNC(),
                        mDevice.mCell.getLAC(),
                        mDevice.mCell.getCID(),
                        mDevice.mCell.getPSC(),
                        0,//T3212 TODO where are we getting these values eva?
                        0,//A5x   TODO where are we getting these values eva?
                        0,//ST_id TODO where are we getting these values eva?
                        MiscUtils.getCurrentTimeStamp(),
                        MiscUtils.getCurrentTimeStamp()
                        );
                    */
 /*               dbHelper.insertDbiMeasure(
                        mDevice.mCell.getCID(),
                        null,
                        MiscUtils.getCurrentTimeStamp(),
                        String.valueOf(mDevice.mCell.getLat()),         //gpsd_lat
                        String.valueOf(mDevice.mCell.getLon()),         //gpsd_lon
                        (int)mDevice.mCell.getAccuracy(),               //gpsd_acc
                        String.valueOf(mDevice.mCell.getLat()),         //gpse_lat TODO where are we getting exact gps data? does this need to be insert when bts first added or later?
                        String.valueOf(mDevice.mCell.getLon()),         //gpse_lon
                        String.valueOf(mDevice.mCell.getSpeed()),       //speed
                        String.valueOf(mDevice.mCell.getDBM()),//bb_power
                        null,//bb_rf_temp
                        String.valueOf(mDevice.mCell.getDBM()),//tx_power
                        String.valueOf(mDevice.mCell.getRssi()),//rx_signal
                        null,//rx_stype
                        null,//BCCH
                        null,//TMSI
                        0,//TA Timing Advance
                        0,//PD
                        (int)mDevice.mCell.getBearing(),//BER
                        null,//AvgEcNo
                        1,//isSubmitted
                        0//isNeighbour
                        );
 ...
 /*           if (mTrackingCell) {
                dbHelper.open();
                //TODO can you tell me where to send this data eva
                // LOCATION_TABLE (locationinfo)    ==>  DBi_measure + DBi_bts
                dbHelper.insertLocation(
                        mDevice.mCell.getLAC(),     // Lac      DBi_bts
                        mDevice.mCell.getCID(),     // CellID   DBi_bts
                        mDevice.mCell.getNetType(), // Net      DBi_bts
                        mDevice.mCell.getLat(),     // Lat      DBi_measure>
                        mDevice.mCell.getLon(),     // Lng      DBi_measure>
                        mDevice.mCell.getDBM(),     // Signal   DBi_measure>
                        mDevice.getCellInfo()       // Connection //TODO where is this data going db?
                );

                // CELL_TABLE                       (cellinfo)      ==>  DBi_measure + DBi_bts
                dbHelper.insertCell(
                        mDevice.mCell.getLAC(),     // Lac          < this is already inserted with insertLocation DBi_bts
                        mDevice.mCell.getCID(),     // CellID       DBi_measure>bts_id
                        mDevice.mCell.getNetType(), // Net          DBi_bts
                        mDevice.mCell.getLat(),     // Lat          DBi_measure>gpsd_lat
                        mDevice.mCell.getLon(),     // Lng          DBi_measure>gpsd_lon
                        mDevice.mCell.getDBM(),     // Signal       DBi_measure>rx_signal   < is this right
                        mDevice.mCell.getMCC(),     // Mcc          DBi_bts>
                        mDevice.mCell.getMNC(),     // Mnc          DBi_bts>
                        mDevice.mCell.getAccuracy(),// Accuracy     DBi_measure>gpsd_accuracy
                        mDevice.mCell.getSpeed(),   // Speed        DBi_measure>spped
                        mDevice.mCell.getBearing(), // Direction    DBi_measure>BER   < is this right
                        mDevice.getNetworkTypeName(),// NetworkType TODO where is this going in db?
                        SystemClock.currentThreadTimeMillis() // MeasurementTaken [ms] TODO where is this going in db?
                );
                dbHelper.close();
            }*/
            }
