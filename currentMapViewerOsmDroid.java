...
*/
                        ...
                        // The indexing here is that of the Cursor and not the DB table itself:
                        // CellID,Lac,Mcc,Mnc,Lat,Lng,AvgSigStr,Samples
                        final int cellID = c.getInt(0);                            // CellID
                        final int lac = c.getInt(1);                               // Lac
                        final int mcc = c.getInt(2);                               // Mcc
                        final int mnc = c.getInt(3);                               // Mnc
                        final double dlat = Double.parseDouble(c.getString(4));    // Lat
                        final double dlng = Double.parseDouble(c.getString(5));    // Lon
                        final GeoPoint location = new GeoPoint(dlat, dlng);        //
                                                                                   //where is c.getString(6)AvgSigStr
                        final int samples = c.getInt(7);                           //Samples
